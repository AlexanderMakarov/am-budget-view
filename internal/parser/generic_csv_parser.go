...
// Expected headers in the exact order matching model.Transaction struct fields
...
func (p GenericCsvFileParser) ParseRawTransactionsFromFile(
	...
	for {
		...
		// OriginCurrencyAmount (model.MoneyWith2DecimalPlaces)
		if originAmount := strings.TrimSpace(record[8]); originAmount != "" {
			var originCurrencyAmount model.MoneyWith2DecimalPlaces
