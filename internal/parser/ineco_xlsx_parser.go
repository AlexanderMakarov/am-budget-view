...
type InecoXlsxTransaction struct {
	Date               time.Time
	AmountOrigCur      model.MoneyWith2DecimalPlaces
	Currency           string
	NotNormalizedEntry model.MoneyWith2DecimalPlaces
	Income             model.MoneyWith2DecimalPlaces
	Expense            model.MoneyWith2DecimalPlaces
	ExchangeRate       model.MoneyWith2DecimalPlaces
...
func (p InecoExcelFileParser) ParseRawTransactionsFromFile(
	...
	transactions := make([]model.Transaction, 0, len(inecoXlsxTransactions))
	for _, t := range inecoXlsxTransactions {
		...
		accountAmount := -t.Expense.Cents
		// If is income then change values.
		if !isExpense {
			accountAmount = t.Income.Cents
		...
		transaction := model.Transaction{
			...
			Date:                 t.Date,
			Details:              t.Details,
			Amount:               model.MoneyWith2DecimalPlaces{Cents: accountAmount},
			OriginCurrencyAmount: model.MoneyWith2DecimalPlaces{Cents: t.AmountOrigCur.Cents},
			Source:               &source,
			AccountCurrency:      accountCurrency,
			FromAccount:          from,
			ToAccount:            to,
...
func parseAmount(rowIndex int, cells []*xlsx.Cell, cellIndex int, name string) (model.MoneyWith2DecimalPlaces, error) {
	var result model.MoneyWith2DecimalPlaces
