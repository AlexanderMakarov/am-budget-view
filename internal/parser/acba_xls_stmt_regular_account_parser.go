...
func (p AcbaRegularAccountExcelFileParser) ParseRawTransactionsFromFile(
	...
	var accountNumber = ""
	var accountCurrency = ""
	var source model.TransactionsSource
	...
	for i := 0; i <= firstSheet.GetNumberRows(); i++ {
		...
		if amountInt := cells[2].GetInt64(); amountInt != 0 {
			amount.Cents = int(amountInt * 100)
		} else {
		...
		creditAmount := model.MoneyWith2DecimalPlaces{}
		...
		debitAmount := model.MoneyWith2DecimalPlaces{}
		...
		if debitAmount.Cents != 0 {
			isExpense = true
			from = receiverSenderAccountNumber
			to = accountNumber
			originCurrencyAmount = model.MoneyWith2DecimalPlaces{Cents: -debitAmount.Cents}
		}
