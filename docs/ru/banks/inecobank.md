...
Для другого конфига приложения используйте `--config tmp-my.yaml`. Скрипт берёт из него `inecobankStatementXmlFilesGlob`; по умолчанию читает `config.yaml` из текущего каталога, если файл существует. Шаблон выбирает файлы, а номер счёта и период читаются из XML. Относительный шаблон, как в Go-приложении, считается от текущего рабочего каталога. XLSX не учитывается.

Список счетов и запрашиваемые даты остаются в конфиге загрузчика. Другой файл можно выбрать через `--download-config path/to/downloads.yaml`; относительные пути загрузчика считаются от каталога этого файла. Параметр `--config` не переносит настройки `bankDownloads` в Python-загрузчик. Без конфига приложения используются `folder_path` и `statement_glob` загрузчика.
