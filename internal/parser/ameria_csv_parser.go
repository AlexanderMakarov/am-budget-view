...
type AmeriaBusinessTransaction struct {
	...
	Account             string
	Credit              model.MoneyWith2DecimalPlaces
	CreditAmd           model.MoneyWith2DecimalPlaces
	Debit               model.MoneyWith2DecimalPlaces
...
func (p AmeriaCsvFileParser) ParseRawTransactionsFromFile(
	...
	for {
		...
		var credit, debit model.MoneyWith2DecimalPlaces
		...
		if credit.Cents == 0 && debit.Cents == 0 {
			continue
		}
	...
	sourceType := model.TransactionsSource{
	...
	transactions := make([]model.Transaction, len(csvTransactions))
	for i, t := range csvTransactions {
		...
		if t.CreditAmd.Cents > 0 || t.DebitAmd.Cents > 0 {
...
var _ model.FileParser = AmeriaCsvFileParser{}
