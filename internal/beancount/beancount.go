...
func BuildBeancountFile(
	...
	currencies map[string]*currency.CurrencyStatistics,
	...
	for _, account := range accounts {
		if account.Source != nil {
			fmt.Fprintf(
				file,
				"%s open Assets:%s:%s\n",
				account.From.Format(model.OutputDateFormat),
				account.Source.Tag,
				account.Number,
			)
		}
	...
	for _, je := range journalEntries {
		// Validate currencies.
		if je.AccountCurrency != "" && !currency.CheckCurrency(je.AccountCurrency) {
			return 0, errors.New(
		...
		if je.OriginCurrency != "" && !currency.CheckCurrency(je.OriginCurrency) {
			return 0, errors.New(
		...
		sb.WriteString(fmt.Sprintf("%s * \"%s\"\n", je.Date.Format(model.OutputDateFormat), je.Details))
		...
		isAccountAmount := len(je.AccountCurrency) > 0 && je.AccountCurrencyAmount.Cents != 0
		isOriginCurAmount := len(je.OriginCurrency) > 0 && je.OriginCurrencyAmount.Cents != 0
