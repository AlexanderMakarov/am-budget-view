...
func Test_NewGroupExtractorByCategories(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"no_accounts",
			map[string]*model.AccountStatistics{},
			nil,
			map[string]struct{}{},
		},
		{
		...
		},
		{
			...
			&config.Config{MyAccounts: []string{"m1", "m2"}},
...
func Test_groupExtractorByCategories_HandleJournalEntry(t *testing.T) {
	source := &model.TransactionsSource{TypeName: "t1", FilePath: "s1"}
	a1Mine := &model.AccountStatistics{Number: "a1", IsTransactionAccount: true}
	a1NotMine := &model.AccountStatistics{Number: "a1", IsTransactionAccount: false}
	a2NotMine := &model.AccountStatistics{Number: "a2", IsTransactionAccount: false}
	a3NotMine := &model.AccountStatistics{Number: "a3", IsTransactionAccount: false}
	jesVariousCategories := []model.JournalEntry{
...
Statistics for        2024-10-27..2024-10-28 (in    USD):
  ...
  Expenses (total  3 groups, filt-ed sum           0.33):
    ...
    c                                    :         0.08, from 2 transaction(s):
      ...
      2024-10-27	Expense	        0.10 	c	a2->a1	t1	s1	'-10c'	0.10 USD (0)
`
	jesVariousCurrencies := []model.JournalEntry{
		{
			IsExpense:             true,
			Date:                  date1,
			Source:                source,
			...
			ToAccount:             "a2",
			AccountCurrency:       "USD",
			AccountCurrencyAmount: model.MoneyWith2DecimalPlaces{Cents: 1},
			OriginCurrency:        "USD",
			OriginCurrencyAmount:  model.MoneyWith2DecimalPlaces{Cents: 1},
			Amounts: map[string]model.AmountInCurrency{
				"USD": {Currency: "USD", Amount: model.MoneyWith2DecimalPlaces{Cents: 1}},
				"AMD": {Currency: "AMD", Amount: model.MoneyWith2DecimalPlaces{Cents: 400}},
			},
		},
		{
			IsExpense:             true,
			...
			ToAccount:             "a2",
			AccountCurrency:       "USD",
			AccountCurrencyAmount: model.MoneyWith2DecimalPlaces{Cents: 2},
			OriginCurrency:        "AMD",
			OriginCurrencyAmount:  model.MoneyWith2DecimalPlaces{Cents: 800},
			Amounts: map[string]model.AmountInCurrency{
				"USD": {Currency: "USD", Amount: model.MoneyWith2DecimalPlaces{Cents: 2}},
				"AMD": {Currency: "AMD", Amount: model.MoneyWith2DecimalPlaces{Cents: 800}},
			},
		},
		{
			IsExpense:             true,
			...
			ToAccount:             "a2",
			AccountCurrency:       "AMD",
			AccountCurrencyAmount: model.MoneyWith2DecimalPlaces{Cents: 1200},
			OriginCurrency:        "USD",
			OriginCurrencyAmount:  model.MoneyWith2DecimalPlaces{Cents: 3},
			Amounts: map[string]model.AmountInCurrency{
				"USD": {Currency: "USD", Amount: model.MoneyWith2DecimalPlaces{Cents: 3}},
				"AMD": {Currency: "AMD", Amount: model.MoneyWith2DecimalPlaces{Cents: 1200}},
			},
		},
		{
			IsExpense:             true,
			...
			ToAccount:             "a2",
			AccountCurrency:       "AMD",
			AccountCurrencyAmount: model.MoneyWith2DecimalPlaces{Cents: 1600},
			Amounts: map[string]model.AmountInCurrency{
				"USD": {Currency: "USD", Amount: model.MoneyWith2DecimalPlaces{Cents: 4}},
				"AMD": {Currency: "AMD", Amount: model.MoneyWith2DecimalPlaces{Cents: 1600}},
			},
		},
		{
			IsExpense:             false,
			...
			ToAccount:             "a2",
			AccountCurrency:       "AMD",
			AccountCurrencyAmount: model.MoneyWith2DecimalPlaces{Cents: 2000},
			OriginCurrency:        "USD",
			OriginCurrencyAmount:  model.MoneyWith2DecimalPlaces{Cents: 5},
			Amounts: map[string]model.AmountInCurrency{
				"USD": {Currency: "USD", Amount: model.MoneyWith2DecimalPlaces{Cents: 5}},
				"AMD": {Currency: "AMD", Amount: model.MoneyWith2DecimalPlaces{Cents: 2000}},
			},
		},
	}
...
`
	tests := []struct {
		...
		},
		{
			...
			accounts:       map[string]*model.AccountStatistics{"a1": a1NotMine, "a2": a2NotMine, "a3": a3NotMine},
			config:         nil,
			journalEntries: jesVariousCategories,
			expected:       expectedVariousCategoriesFull,
		},
		{
			...
			accounts:       map[string]*model.AccountStatistics{"a1": a1Mine},
			config:         nil,
			journalEntries: jesVariousCategories,
		...
		},
		{
			...
			accounts:       map[string]*model.AccountStatistics{"a1": a1NotMine, "a2": a2NotMine, "a3": a3NotMine},
			config:         &config.Config{MyAccounts: []string{"a1"}},
			journalEntries: jesVariousCategories,
			expected:       expectedVariousCategoriesA1Mine,
		},
		{
...
func newUsdJE(amount int, isExpense bool, category, from, to string, source *model.TransactionsSource) model.JournalEntry {
	sign := "+"
	if isExpense {
		sign = "-"
	}
	return model.JournalEntry{
		...
		AccountCurrencyAmount: model.MoneyWith2DecimalPlaces{Cents: amount},
		FromAccount:           from,
		ToAccount:             to,
		Amounts:               map[string]model.AmountInCurrency{"USD": {Currency: "USD", Amount: model.MoneyWith2DecimalPlaces{Cents: amount}}},
	}
}
