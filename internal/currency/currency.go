...
func atLeast1CentDiv(amountCents int, rate float64) model.MoneyWith2DecimalPlaces {
	n := int(float64(amountCents) / rate)
	if n == 0 {
		n = 1
	}
	return model.MoneyWith2DecimalPlaces{Cents: n}
}

func atLeast1CentMul(amountCents int, rate float64) model.MoneyWith2DecimalPlaces {
	n := int(float64(amountCents) * rate)
	if n == 0 {
		n = 1
	}
...
type CurrencyStatistics struct {
	...
	OverlappedWithOtherCurrencyAmount model.MoneyWith2DecimalPlaces
	// TotalAmount is a total amount of the currency.
	TotalAmount model.MoneyWith2DecimalPlaces
...
type DataMart struct {
	...
	Accounts map[string]*model.AccountStatistics
...
// ValidCurrencyRegex validates a currency name per beancount rules.
var ValidCurrencyRegex = regexp.MustCompile(`^[A-Z][A-Z0-9'._-]{0,22}[A-Z0-9]$`)

// CheckCurrency checks if a currency name is valid per beancount rules.
...
// model.Transaction details may contain it in formats
// - "330000 AMD / 4.4 = 75000 RUB"
// - "1550 EUR * 410.84 = 636802 AMD"
// - "1085500 AMD / 417.5 = 2600 EUR"
func parseExchangeRateFromDetails(date time.Time, details string, targetCurrency1, targetCurrency2 string, source *model.TransactionsSource) *ExchangeRate {
...
func printCurrencyStatisticsMap(convertbleCurrencies map[string]*CurrencyStatistics) {
	...
	for currency, stat := range convertbleCurrencies {
		fmt.Printf("  %s\t%s\t%s\t%d\n",
			currency,
			stat.From.Format(model.OutputDateFormat),
			stat.To.Format(model.OutputDateFormat),
...
func convertToCurrency(
...
) (model.MoneyWith2DecimalPlaces, int, []string) {
	...
	if exchangeRateDirect != nil {
		...
		if exchangeRateDirect.source != nil && exchangeRateDirect.source.TypeName == model.ConstantExchangeRateSourceName {
			precision = model.ConstantExchangeRatePrecision
		}
		if exchangeRateDirect.currencyTo == targetCurrency {
			return atLeast1CentDiv(amount.Cents, exchangeRateDirect.exchangeRate),
				precision,
				[]string{
					buildConversionPath(
		...
		return atLeast1CentMul(amount.Cents, exchangeRateDirect.exchangeRate),
			precision,
			[]string{
				buildConversionPath(
					exchangeRateDirect.currencyTo,
	...
	for currency := range curStates {
		nodes[currency] = &currencyNode{
			currency:  currency,
			amount:    model.MoneyWith2DecimalPlaces{Cents: math.MaxInt},
	...
	for len(queue) > 0 {
		...
		for _, er := range fromCurState.statistics.ExchangeRates {
			...
			// Special handling for constant exchange rates.
			if er.source != nil && er.source.TypeName == model.ConstantExchangeRateSourceName {
				stepPrecision = model.ConstantExchangeRatePrecision
			}
			...
			if newPrecision < otherNode.precision {
				...
				if current.currency == er.currencyFrom {
					newAmount = atLeast1CentDiv(fromNode.amount.Cents, er.exchangeRate)
					...
					newAmount = atLeast1CentMul(fromNode.amount.Cents, er.exchangeRate)
					pathEntry = buildConversionPath(
						er.currencyTo,
						er.currencyFrom,
						1/er.exchangeRate,
	...
	if targetNode, exists := nodes[targetCurrency]; exists {
		if targetNode.amount.Cents != math.MaxInt {
...
func BuildDataMart(
	...
	convertibleCurrencies, err := buildConvertibleCurrencies(currencies, cfg)
	if err != nil {
		return nil, err
	}
...
func findAccountsAndCurrencies(transactions []model.Transaction) (map[string]*model.AccountStatistics, map[string]*CurrencyStatistics, error) {
	...
	accounts := make(map[string]*model.AccountStatistics)
	...
	for _, t := range transactions {
			...
			if ValidCurrencyRegex.MatchString(t.AccountCurrency) {
				...
				if !ok {
					accountCurrency = &CurrencyStatistics{
						...
						Transactions:  []*model.Transaction{&t},
						ExchangeRates: []*ExchangeRate{},
					}
				...
				if t.OriginCurrency != "" {
					accountCurrency.OverlappedWithOtherCurrencyAmount.Cents += t.Amount.Cents
					...
					if t.Amount.Cents != 0 && t.OriginCurrencyAmount.Cents != 0 {
						exchangeRate = &ExchangeRate{
							date:         t.Date,
							currencyFrom: t.AccountCurrency,
							currencyTo:   t.OriginCurrency,
							exchangeRate: float64(t.Amount.Cents) / float64(t.OriginCurrencyAmount.Cents),
							source:       t.Source,
						}
						accountCurrency.ExchangeRates = append(accountCurrency.ExchangeRates, exchangeRate)
					}
				}
				accountCurrency.TotalAmount.Cents += t.Amount.Cents
		...
		if t.OriginCurrency != "" {
			if ValidCurrencyRegex.MatchString(t.OriginCurrency) {
				...
				if t.AccountCurrency != "" {
					originCurrency.OverlappedWithOtherCurrencyAmount.Cents += t.Amount.Cents
				}
				...
				originCurrency.TotalAmount.Cents += t.OriginCurrencyAmount.Cents
				atLeastOneCurrency = true
			} else {
				return nil, nil, errors.New(
		...
		if len(t.ToAccount) > 0 {
			if account, ok := accounts[t.ToAccount]; !ok {
				accounts[t.ToAccount] = &model.AccountStatistics{
					Number:                   t.ToAccount,
		...
		if len(t.FromAccount) > 0 {
			if account, ok := accounts[t.FromAccount]; !ok {
				accounts[t.FromAccount] = &model.AccountStatistics{
...
func buildConvertibleCurrencies(currencies map[string]*CurrencyStatistics, cfg *config.Config) (map[string]*CurrencyStatistics, error) {
	...
	// a) Find rules.
	minTimespanPercent := cfg.MinCurrencyTimespanPercent
	maxGapDays := cfg.MaxCurrencyTimespanGapDays
	...
	if len(cfg.ConvertToCurrencies) > 0 {
		for _, currencyName := range cfg.ConvertToCurrencies {
		...
		if len(cfg.ExchangeRates) > 0 {

			// Create a dummy source for constant exchange rates.
			constantSource := &model.TransactionsSource{
				TypeName: model.ConstantExchangeRateSourceName,
				Tag:      model.ConstantExchangeRateSourceName,
				FilePath: model.ConstantExchangeRateSourceFilePath,
			}

			// Build map of maps of provided exchange rates.
			cfgExchangeRates := make(map[string]map[string]*ExchangeRate)
			for currencyName, currencyMap := range cfg.ExchangeRates {
...
func BuildJournalEntries(
	...
	for _, t := range dataMart.SortedTransactions {
		...
		category, isUncategorized, err := cat.CategorizeTransaction(&t)
		if err != nil {
			return nil, nil, err
		...
		amounts := make(map[string]model.AmountInCurrency, len(dataMart.ConvertibleCurrencies))
		for _, curStatistic := range dataMart.ConvertibleCurrencies {
			var amountAccCur, amountOrgCur model.MoneyWith2DecimalPlaces
			...
			if t.AccountCurrency != "" && t.Amount.Cents != 0 {
			...
			if t.OriginCurrency != "" && t.OriginCurrencyAmount.Cents != 0 {
			...
			if amountAccCur.Cents == 0 && amountOrgCur.Cents == 0 {
			...
			if precisionAccCur <= precisionOrgCur {
				amounts[curStatistic.Name] = model.AmountInCurrency{
