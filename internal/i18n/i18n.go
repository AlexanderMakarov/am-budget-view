...
// global is the singleton I18n instance.
var global *I18n

// Init initializes the global I18n singleton.
func Init(backend I18nFsBackend, defaultLocale string, devMode bool) error {
	global = &I18n{}
	return global.Init(backend, defaultLocale, devMode)
}

// T translates a key using the global singleton.
func T(key string, args ...interface{}) string {
	return global.T(key, args...)
}

// SetLocale sets the locale on the global singleton.
func SetLocale(locale string) error {
	return global.SetLocale(locale)
}

// GetLocale returns the current locale of the global singleton.
...
type I18nFsBackend struct {
	...
	FS      fs.FS
	DevMode bool
	// RootDir is a prefix for locales path inside FS. Default "" means locales are at "locales/".
	// Set to e.g. "testdata" if locales are embedded at "testdata/locales/".
	RootDir string
}

func (b *I18nFsBackend) localesDir() string {
	if b.RootDir != "" {
		return b.RootDir + "/locales"
	}
	return "locales"
}
...
func (b *I18nFsBackend) GetLocales() ([]string, error) {
	...
	if b.DevMode {
		...
		if err != nil {
			return nil, fmt.Errorf("can't read locales from '%s' directory: %w", locDir, err)
		}
	} else {
		entries, err = fs.ReadDir(b.FS, locDir)
		if err != nil {
...
func (b *I18nFsBackend) LoadTranslations(defaultLang string) (map[string]map[string]interface{}, error) {
	...
	for _, locale := range locales {
		filePath := locDir + "/" + locale + "/translation.json"
		var data []byte
		var err error
		if b.DevMode {
...
func (i *I18n) Init(backend I18nFsBackend, defaultLocale string, devMode bool) error {
	i.backend = backend
	i.locale = defaultLocale
	i.devMode = devMode
	var err error
	i.translations, err = i.backend.LoadTranslations(defaultLocale)
	if err != nil {
	...
	i.funcs = i.buildDefaultFormatters()
	return nil
}

// RegisterFunc registers a custom formatter function.
func (i *I18n) RegisterFunc(name string, function func(entry interface{}, props map[string]interface{}) string) {
	i.funcs[name] = function
}

// buildDefaultFormatters builds default formatters.
func (i *I18n) buildDefaultFormatters() map[string]func(val interface{}, props map[string]interface{}) string {
	...
	result["currency"] = func(val interface{}, props map[string]interface{}) string {
		var amount model.MoneyWith2DecimalPlaces
		switch v := val.(type) {
		case model.MoneyWith2DecimalPlaces:
			amount = v
		case float64:
			amount = model.MoneyWith2DecimalPlaces{Cents: int(v * 100)}
		default:
			return fmt.Sprintf("%+v", val)
		}
...
func (i *I18n) validateKeys() {
	keysInLocales := make(map[string][]string)
	locales := []string{}
	// Collect keys and locales.
	for locale, translations := range i.translations {
...
func (i *I18n) T(key string, args ...interface{}) string {
	var entry interface{}
	var ok bool
	if entry, ok = i.translations[i.locale][key]; !ok {
		return i.Tfallback("missed key", key, args...)
	}
	...
	for idx, arg := range args {
		if idx%2 == 0 {
			if argKey, ok = arg.(string); !ok {
				return i.Tfallback(fmt.Sprintf("wrong call - odd argument '%s' is not a string", arg), key, args...)
			}
		} else {
			props[argKey] = arg
		}
		...
		for _, match := range matches {
			...
			if idx := strings.Index(formatterSpec, "("); idx != -1 {
				...
				if !strings.HasSuffix(formatterSpec, ")") {
					return i.Tfallback(fmt.Sprintf("malformed formatter call '%s' - missing closing bracket", formatterSpec), key, args...)
				}
				...
				if optionsStr != "" {
					...
					for _, pair := range optionPairs {
						...
						if len(kv) != 2 {
							return i.Tfallback(fmt.Sprintf("malformed option '%s' in '%s' formatter call", pair, formatterName), key, args...)
						}
			...
			formatter := i.funcs[formatterName]
			if formatter != nil {
				...
				return i.Tfallback(fmt.Sprintf("'%s' in translation misses '%s' formatter name", parts[0], formatterName), key, args...)
			}
		}
		return result
	...
	case map[string]interface{}:
		...
		if format, exists := v["format"]; exists {
			if formatFunc, ok := i.funcs[format.(string)]; ok {
				return formatFunc(v, props)
			}
			return i.Tfallback("unknown format function", key, args...)
		}
		return i.Tfallback("invalid translation format", key, args...)
	default:
		return i.Tfallback("invalid translation type", key, args...)
	}
}

func (i *I18n) Tfallback(reason, key string, args ...interface{}) string {
	...
	if i.devMode {
		panic(fmt.Sprintf("[%s: %s] %s, %s", i.locale, reason, key, strings.Join(argsList, ", ")))
	}
	return fmt.Sprintf("[%s: %s] %s, %s", i.locale, reason, key, strings.Join(argsList, ", "))
}

// number formats a number based on the specified options.
func (i *I18n) number(value interface{}, props map[string]interface{}) string {
	var val model.MoneyWith2DecimalPlaces
	switch v := value.(type) {
	case model.MoneyWith2DecimalPlaces:
		val = v
	case float64:
		val = model.MoneyWith2DecimalPlaces{Cents: int(v * 100)}
	default:
		return fmt.Sprintf("%+v", value)
	}
